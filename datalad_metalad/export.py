...
def export_metadata(tree_version_list: TreeVersionList,
    ...
    write_version(destination / "version.json")
    for uuid in uuid_set.uuids():
        export_uuid(uuid, uuid_set, destination)
    yield {"exported": "all"}
...
def export_uuid(uuid: str,
                uuid_set: UUIDSet,
                destination: Path):

    uuid_path, uuid_file = get_dir_for(str(uuid), (2, 2))
    destination = destination / uuid_path / uuid_file

    version_list = uuid_set.get_version_list(uuid)
    for version, versioned_element in version_list.get_versioned_elements():
        time_stamp, dataset_path, mappable_object = versioned_element
        mrr = cast(MetadataRootRecord, mappable_object)

        version_path, version_file = get_dir_for(version, (2,))
        version_path = destination / version_path / version_file
        version_path.mkdir(parents=True, exist_ok=True)
        print(version_path)
        needs_purge = mrr.ensure_mapped()
        dataset_level_path = version_path / "dataset-level-metadata.json"
        export_metadata_instances(dataset_level_path, mrr.dataset_level_metadata)
        file_tree_path = version_path / "file-tree.json"
        file_objects_path = version_path / "file-objects"
        if needs_purge:
            mrr.purge()


        print(dataset_level_path)
        print(file_tree_path)
        print(file_objects_path)


def export_metadata_instances(destination_file_name: Path, metadata: Metadata):
    with ensure_mapped(metadata):
        instance_dict = {
            extractor_name: instance_set.to_json_obj()
            for extractor_name, instance_set in metadata.instance_sets.items()
        }
        destination_file_name.write_text(json.dumps(instance_dict))


def get_dir_for(name: str, parts: Iterable[int]) -> Tuple[Path, Path]:
